...
## 0. GAN?
GAN의 첫 게시글입니다! 너무나도 유명한 논문이고 빠르게 발전하고 있는 GAN 모델들의 초석이 된 논문인 [Generative Adversarial Networks](https://arxiv.org/abs/1406.2661) 논문을 정리해보고자 합니다. 논문 이해 위주의 글이며 Related Work 부분은 제외했습니다.
...
<span style="color:gray">
  Maximum likelihood Estimation(MLE)는 likelihood의 최대값을 선택하는 방법입니다.<br>
  여기서 likelihood는 조건부 확률을 의미하며 $p(z|x)$로 표현할 수 있고 모델 $x$에서 관측값 $z$가 나올 조건부 확률입니다.<br>
  likelihood를 최대화한다는 것은 모델의 추측값이 최대한 데이터와 잘 맞도록 모델의 파라미터를 선택한 다는 것을 의미합니다.
  <br><br>
  참고<br>
  - 다크 프로그래머님의 <a href="https://darkpgmr.tistory.com/62">베이즈 정리, ML과 MAP, 그리고 영상처리</a>
  <br>
  - JINSOL KIM 님의 <a href="https://gaussian37.github.io/ml-concept-mlemap/">MLE(Maximum Likelihood Estimation)와 MAP(Maxima Posterior)에 관하여</a>
...
<span style="color:gray">
  특정 상태의 확률이 오직 과거의 상태에 의존하는 성질을 Markov 성질이라 합니다.<br>
  Markov chain이란 이런 Markov 성질을 가진 이산 확률 과정을 의미합니다.<br>
  이때 차수에 따라 의존하는 과거의 크기가 달라집니다.
  <img src="/assets/images/posts/gan/markov_chain.png" width="400">
  r=1이면 1차 마르코프 체인으로 t시점에서는 오직 t-1시점의 영향만 받습니다.<br>
  r=2이면 2차 마르코프 체인으로 t시점에서는 오직 t-1,  t-2시점의 영향만 받습니다.
  <br><br>

  대표적인 예시로 학생 때 어디선가 많이 본 듯한 문제인 비가 올 확률을 구하는 문제가 있습니다.<br>
  "비가 온 다음 날에 비가 올 확률은 70%이고 비가 오지 않은 다음 날에 비가 올 확률은 20%입니다" 라는 문장을 모델화 한다면 다음과 같은 그림이 나오게 됩니다. 이 문제의 경우 이전 날에 대한 영향만 받으니 1차 마르코프 모델이 됩니다.
  <br>
  "이미지"

  "확률"
  위와 같은 상태 전이도를 가지고 있을 때 전이 행렬은 A와 같으며 이런 전이 행렬을 가지고 계산하는 것이 Markov chain입니다. 예시로 오늘 비가 온 다음 날 비가 오지 않고 모레 비가 올 확률을 Markov chain을 이용해서 구해보겠습니다.
  <br><br>

  $\begin{align}
  P(O|A) &= P(o_{1}=비 O, o_2=비 X, o_3=비 O | A) \\ &= P(o_2|o_1) \cdot P(o_3|o_2) = P(비 X | 비 O) \cdot P(비 O | 비 X) \\ &= 0.3 \cdot 0.2 = 0.06
  \end{align}$

  <br><br>

  참고<br>
  - ratsgo 님의 <a href="https://ratsgo.github.io/machine%20learning/2017/03/18/HMMs/">은닉마코프모델(Hidden Markov Models)</a><br>
  - 티랩 님의 <a href="https://t-lab.tistory.com/18">HMM(Hidden Markov Model) 2. 마르코프 모델</a>
  <br>
</span>
</details>
...
<br>
![table1](/assets/images/posts/gan/table1.png)
...
<span style="color:gray">
  <img src="/assets/images/posts/gan/helvetica_scenario.png" width="500">
