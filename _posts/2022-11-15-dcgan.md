...
Generator의 경우 논문의 Figure 1 그림을 통해 구조를 파악할 수 있습니다. Generator는 100차원의 $z$를 reshape해 (channel, height, width) 순으로 (1024, 4, 4)로 만듭니다. 이후 fractionally strided convoltion을 이용해 채널 수를 줄이고 이미지 높이와 너비 값을 늘리는 작업을 4번 반복합니다. 이미지의 크기는 $z$ -> (1024, 4, 4) -> (512, 8, 8) -> (256, 16, 16) -> (128, 32, 32) -> (3, 64, 64) 순으로 변화하며 (3, 64, 64)의 이미지가 64 x 64 크기의 rgb 이미지로 출력됩니다.

구조 외에 논문에서 generator 구현에 대해 언급된 사항들은 아래와 같으면 해당 내용들을 코드로 구현했습니다.
- 4번의 fractionally strided convolution() 사용
- 출력 레이어를 제외한 모든 레이어에 batch normalize 사용
- relu 사용
- tanh 사용
...
Discriminator는 Generator와는 달리 모델의 구조가 Figure로 나와있거나 하지 않았습니다. Generator에서는 transposed convolution을 썼다면 Discriminator는 convolution을 사용하니 레이어 수를 맞추어 generator에 대칭하는 구조로 만들었습니다. 또한 generator와 마찬가지로 논문에 언급된 내용들이 있어 해당 내용을 만족할 수 있도록 코드를 구현해보았습니다.
- convolution 이후 fully connected layer를 제거
- 입력 레이어를 제외한 모든 레이어에 batch normalize 사용
- 경사도는 0.2의 leaky relu 사용
- 마지막 convolutiokn 이후 flatten되어 단일 sigmoid 출력
...
### 3.1 사용 설정
 ...
 논문에서 언급된 설정들은 다음과 같습니다.
 - $z$는 100차원의 uniform distribution
 - 128 mini batch, Stochastic gradient descent 사용
 - 모델의 가중치 초기화
 - adam optimizer를 사용하며 learning rate로 0.0002 값을 사용하며 $\beta_1 = 0.5$로 설정
